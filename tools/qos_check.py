...
def check():
    ...
    for (topic_name, topic_type) in node.get_topic_names_and_types():
        ...
        if not args.unmatched or \
           len(info_map[TopicEndpointTypeEnum.PUBLISHER]['reliability'].union(info_map[TopicEndpointTypeEnum.SUBSCRIPTION]['reliability'])) != 1 or \
           len(info_map[TopicEndpointTypeEnum.PUBLISHER]['durability'].union(info_map[TopicEndpointTypeEnum.SUBSCRIPTION]['durability'])) != 1:
